...
export default class Header extends IndentedBlock {
	...
	static isValidOverlined(inputInterface, context, parsedInterface) {
		...
		return (
			(!currentBlock || !currentBlock.isParagraph) &&
			overline === underline &&
	...
	static isValidUnderlined(inputInterface, context, parsedInterface) {
		...
		const matchesOpenHeader =
			context.openHeader &&
			context.openHeader.char === char &&
			context.openHeader.length === underline.length;
		//If there is no open header and we are the same length as the text we match it
		const matchesText =
			!context.openHeader && isValidHeaderLineLength(text, underline);

		return (
			currentBlock &&
			currentBlock.isParagraph &&
			currentBlock.isOneLine && //if the current block is one line of text
			(matchesOpenHeader || matchesText)
		); //and we match the open header or the text
	}

	static isNextBlock(inputInterface, context, parsedInterface) {
		return (
			this.isValidHeader(inputInterface) &&
			(this.isValidOverlined(inputInterface, context, parsedInterface) ||
				this.isValidUnderlined(
					inputInterface,
					context,
					parsedInterface
				))
	...
	static parse(inputInterface, context, parsedInterface) {
		...
		if (!context.headerLevels.charToLevel[char]) {
			context.headerLevels.currentLevel = Math.min(
				6,
				context.headerLevels.currentLevel + 1
			);
			context.headerLevels.charToLevel[char] =
				context.headerLevels.currentLevel;
		}
	...
	getOutput(context, options) {
		...
		const { output, context: newContext } = textBlock.getOutput(
			context,
			true
		);
		const data = output.data
			? { ...this.blockData, ...output.data, char }
			: { char };
