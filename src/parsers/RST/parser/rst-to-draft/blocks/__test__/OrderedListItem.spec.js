...
describe('OrderedListItem', () => {
	describe('isNextBlock', () => {
		test('Matches Roman Numerals', () => {
			...
			for (let test of tests) {
				...
				expect(
					OrderedListItem.isNextBlock(inputInterface)
				).toBeTruthy();
			}
		});
	...
	describe('Instance Tests', () => {
		test('Should append paragraphs that are the same offset', () => {
			const orderedListItem = new OrderedListItem(
				'1. Ordered List Item',
				'1.'
			);
		...
		test('Appending text adds it to the previous line', () => {
			const orderedListItem = new OrderedListItem('1. Ordered', '1', {
				text: new Text('Ordered'),
			});
			const paragraph = new Paragraph('  List Item', '', {
				text: new Text('List Item'),
			});

			orderedListItem.appendBlock(paragraph);

			expect(orderedListItem.parts.text.text).toEqual(
				'Ordered List Item'
			);
		});
	});
