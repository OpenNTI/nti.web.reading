	...
	describe('Instance tests', () => {
		...
		describe('shouldAppendBlock', () => {
			test('Should append deeper blocks', () => {
				const paragraph = new Paragraph('  indented paragraph', '', {
			...
			test('Should not append blocks that are the same depth', () => {
				const paragraph = new Paragraph('not indented paragraph', '', {
					text: 'indented paragraph',
				});
		...
		describe('appendBlock', () => {
			test('Field Lists are added to the options', () => {
				const name = 'name';
				const value = 'value';
				const fieldList = new FieldList('  :name: value', '', {
					name,
					value,
				});
			...
			test('Non empty blocks are appended to the body', () => {
				const first = new Paragraph('  paragraph 1', '', {
					test: 'paragraph 1',
				});
				const second = new Paragraph('  paragraph 2', '', {
					test: 'paragraph 2',
				});
