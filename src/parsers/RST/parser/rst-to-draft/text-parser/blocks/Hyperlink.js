...
export default class Target {
	static isNextBlock(inputInterface, context, parsedInterface) {
		...
		return (
			input === '_' && //Check that its an underscore
			!context.openRange && //and we aren't parsing a range
			(!currentBlock ||
				currentBlock.isInterpreted ||
				currentBlock.isPlaintext) && //and if there is a one the current block is linkable
			Regex.isValidRangeEnd(prevInput, nextInput)
		); //and that we are a valid end of a range
	}
	...
	static parse(inputInterface, context, parsedInterface) {
		...
		if (
			currentBlock &&
			(currentBlock.isInterpreted || currentBlock.isPlaintext)
		) {
	...
	getOutput(context) {
		return this[MARKER_FOR] &&
			(this[MARKER_FOR].isValidRange || this[MARKER_FOR].isPlaintext)
			? null
	...
	getOutputForInterpreted(block, context) {
		...
		const entity = key &&
			href && {
				...
				data: { name, href, contiguous: false },
			};
